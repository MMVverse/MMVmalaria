...
#' @return A list containing the IQR model spec, see `modelSpec_IQRest`
#' @export
...
backwardElimination <- function(model, dosing,
  ...
  if (!is_IQRmodel(model)) {
    stop("'model' should be a IQRmodel object.")
  }

  # Check if covariateToTest is not empty:
  if (length(covariateToTest) == 0) {
    stop("'covariateToTest' is empty.")
  }
  ...
  # General setting:
  if ("multiTestN" %in% names(setting)) {
    multiTestN <- setting$multiTestN
  } else {
    multiTestN <- 1
  }
  if ("multiTestSD" %in% names(setting)) {
    multiTestSD <- setting$multiTestSD
  } else {
    multiTestSD <- 0.5
  }
  if ("FLAGanalytic" %in% names(setting)) {
    FLAGanalytic <- setting$FLAGanalytic
  } else {
    FLAGanalytic <- TRUE
  }
  if ("keepProjectFolder" %in% names(setting)) {
    keepProjectFolder <- setting$keepProjectFolder
  } else {
    keepProjectFolder <- FALSE
  }

  # General algo. options:
  if ("algOpt.SEED" %in% names(setting)) {
    algOpt.SEED <- setting$algOpt.SEED
  } else {
    algOpt.SEED <- 123456
  }
  if ("algOpt.K1" %in% names(setting)) {
    algOpt.K1 <- setting$algOpt.K1
  } else {
    algOpt.K1 <- 500
  }
  if ("algOpt.K2" %in% names(setting)) {
    algOpt.K2 <- setting$algOpt.K2
  } else {
    algOpt.K2 <- 200
  }
  if ("algOpt.NRCHAINS" %in% names(setting)) {
    algOpt.NRCHAINS <- setting$algOpt.NRCHAINS
  } else {
    algOpt.NRCHAINS <- min(ceiling(50 / Nsubj), 10)
  }

  # NONMEM algo. options:
  if ("algOpt.NONMEM.METHOD" %in% names(setting)) {
    algOpt.NONMEM.METHOD <- setting$algOpt.NONMEM.METHOD
  } else {
    algOpt.NONMEM.METHOD <- "SAEM"
  }
  if ("algOpt.NONMEM.MAXEVAL" %in% names(setting)) {
    algOpt.NONMEM.MAXEVAL <- setting$algOpt.NONMEM.MAXEVAL
  } else {
    algOpt.NONMEM.MAXEVAL <- 9999
  }
  if ("algOpt.NONMEM.SIGDIGITS" %in% names(setting)) {
    algOpt.NONMEM.SIGDIGITS <- setting$algOpt.NONMEM.SIGDIGITS
  } else {
    algOpt.NONMEM.SIGDIGITS <- 3
  }
  if ("algOpt.NONMEM.PRINT" %in% names(setting)) {
    algOpt.NONMEM.PRINT <- setting$algOpt.NONMEM.PRINT
  } else {
    algOpt.NONMEM.PRINT <- 1
  }
  if ("algOpt.NONMEM.COVSTEP_MATRIX" %in% names(setting)) {
    algOpt.NONMEM.COVSTEP_MATRIX <- setting$algOpt.NONMEM.COVSTEP_MATRIX
  } else {
    algOpt.NONMEM.COVSTEP_MATRIX <- "S"
  }
  if ("algOpt.NONMEM.ADVAN7" %in% names(setting)) {
    algOpt.NONMEM.ADVAN7 <- setting$algOpt.NONMEM.ADVAN7
  } else {
    algOpt.NONMEM.ADVAN7 <- TRUE
  }
  if ("algOpt.NONMEM.N1" %in% names(setting)) {
    algOpt.NONMEM.N1 <- setting$algOpt.NONMEM.N1
  } else {
    algOpt.NONMEM.N1 <- 1000
  }
  if ("algOpt.NONMEM.TOL" %in% names(setting)) {
    algOpt.NONMEM.TOL <- setting$algOpt.NONMEM.TOL
  } else {
    algOpt.NONMEM.TOL <- 6
  }
  if ("algOpt.NONMEM.SIGL" %in% names(setting)) {
    algOpt.NONMEM.SIGL <- setting$algOpt.NONMEM.SIGL
  } else {
    algOpt.NONMEM.SIGL <- NULL
  }
  if ("algOpt.NONMEM.M4" %in% names(setting)) {
    algOpt.NONMEM.M4 <- setting$algOpt.NONMEM.M4
  } else {
    algOpt.NONMEM.M4 <- FALSE
  }
  if ("algOpt.NONMEM.FOCEIOFV" %in% names(setting)) {
    algOpt.NONMEM.FOCEIOFV <- setting$algOpt.NONMEM.FOCEIOFV
  } else {
    algOpt.NONMEM.FOCEIOFV <- FALSE
  }
  if ("algOpt.NONMEM.IMPORTANCESAMPLING" %in% names(setting)) {
    algOpt.NONMEM.IMPORTANCESAMPLING <- setting$algOpt.NONMEM.IMPORTANCESAMPLING
  } else {
    algOpt.NONMEM.IMPORTANCESAMPLING <- TRUE
  }
  if ("algOpt.NONMEM.IMP_ITERATIONS" %in% names(setting)) {
    algOpt.NONMEM.IMP_ITERATIONS <- setting$algOpt.NONMEM.IMP_ITERATIONS
  } else {
    algOpt.NONMEM.IMP_ITERATIONS <- 10
  }
  if ("algOpt.NONMEM.ITS" %in% names(setting)) {
    algOpt.NONMEM.ITS <- setting$algOpt.NONMEM.ITS
  } else {
    algOpt.NONMEM.ITS <- TRUE
  }
  if ("algOpt.NONMEM.ITS_ITERATIONS" %in% names(setting)) {
    algOpt.NONMEM.ITS_ITERATIONS <- setting$algOpt.NONMEM.ITS_ITERATIONS
  } else {
    algOpt.NONMEM.ITS_ITERATIONS <- 10
  }
  if ("algOpt.NONMEM.WRES" %in% names(setting)) {
    algOpt.NONMEM.WRES <- setting$algOpt.NONMEM.WRES
  } else {
    algOpt.NONMEM.WRES <- NULL
  }
  if ("algOpt.NONMEM.PRED" %in% names(setting)) {
    algOpt.NONMEM.PRED <- setting$algOpt.NONMEM.PRED
  } else {
    algOpt.NONMEM.PRED <- NULL
  }
  if ("algOpt.NONMEM.RES" %in% names(setting)) {
    algOpt.NONMEM.RES <- setting$algOpt.NONMEM.RES
  } else {
    algOpt.NONMEM.RES <- NULL
  }

  # Monolix algo. options:
  if ("algOpt.MONOLIX.individualParameters" %in% names(setting)) {
    algOpt.MONOLIX.individualParameters <- setting$algOpt.MONOLIX.individualParameters
  } else {
    algOpt.MONOLIX.individualParameters <- "conditionalMode"
  }
  if ("algOpt.MONOLIX.indivMCMClength" %in% names(setting)) {
    algOpt.MONOLIX.indivMCMClength <- setting$algOpt.MONOLIX.indivMCMClength
  } else {
    algOpt.MONOLIX.indivMCMClength <- 50
  }
  if ("algOpt.MONOLIX.indivNsim" %in% names(setting)) {
    algOpt.MONOLIX.indivNsim <- setting$algOpt.MONOLIX.indivNsim
  } else {
    algOpt.MONOLIX.indivNsim <- 10
  }
  if ("algOpt.MONOLIX.indivRatio" %in% names(setting)) {
    algOpt.MONOLIX.indivRatio <- setting$algOpt.MONOLIX.indivRatio
  } else {
    algOpt.MONOLIX.indivRatio <- 0.05
  }
  if ("algOpt.MONOLIX.logLikelihood" %in% names(setting)) {
    algOpt.MONOLIX.logLikelihood <- setting$algOpt.MONOLIX.logLikelihood
  } else {
    algOpt.MONOLIX.logLikelihood <- "Linearization"
  }
  if ("algOpt.MONOLIX.fim" %in% names(setting)) {
    algOpt.MONOLIX.fim <- setting$algOpt.MONOLIX.fim
  } else {
    algOpt.MONOLIX.fim <- "Linearization"
  }
  if ("algOpt.MONOLIX.variability" %in% names(setting)) {
    algOpt.MONOLIX.variability <- setting$algOpt.MONOLIX.variability
  } else {
    algOpt.MONOLIX.variability <- "FirstStage"
  }
  if ("algOpt.MONOLIX.startTime" %in% names(setting)) {
    algOpt.MONOLIX.startTime <- setting$algOpt.MONOLIX.startTime
  } else {
    algOpt.MONOLIX.startTime <- NULL
  }
  if ("algOpt.MONOLIX.STIFF" %in% names(setting)) {
    algOpt.MONOLIX.STIFF <- setting$algOpt.MONOLIX.STIFF
  } else {
    algOpt.MONOLIX.STIFF <- TRUE
  }

  # NLMIXR algo. options:
  if ("algOpt.NLMIXR.method" %in% names(setting)) {
    algOpt.NLMIXR.method <- setting$algOpt.NLMIXR.method
  } else {
    algOpt.NLMIXR.method <- "SAEM"
  }
  if ("algOpt.NLMIXR.control" %in% names(setting)) {
    algOpt.NLMIXR.control <- setting$algOpt.NLMIXR.control
  } else {
  ...
  while ((p_j_max > alpha) & (length(parameter_i) > 0)) {
    ...
    #   OBJ, AIC, BIC
    goodnessOfFit_i0 <- c(
      "OBJ" = resModel_i0$objectivefunction$OBJ,
    ...
    for (j in 1:length(parameter_i)) {
      ...
      #   OBJ, AIC, BIC
      goodnessOfFit_i[j, ] <- c(
        resModel_ij$objectivefunction$OBJ,
...
compare_DataModelMMV <- function(modelFolder,
  ...
  modelPredPlot <- reshape(modelPred,
    direction = "long",
  ...
  gr <- gr +
    ...
    labs(
      ...
      y = ylabel,
      caption = paste0(
        "Activity: ", MMVbase::get_ActivityPath(ActivityPath),
        "\nModel: ", modelFolder
      )
    ) +
...
compare_DataPopPred <- function(PKmodelFolder,
  ...
  for (modelk in modelFolders) {
    ...
    # Do simulation
    res <- do.call(sim_IQRmodel, c(list(
      model = model,
      simtime = simtime, eventTable = eventTable
...
compare_DataModelTimeRecrudCombo <- function(ModelCombo,
  ...
  if ((IQRversion <= "0.9.99") && (is_IQRsysFit(ModelCombo))) {
    ...
    # Caption:
    caption <- paste0(
      "Model: ", ModelCombo$estObject$model$name, ".txt",
  ...
  for (k in 1:length(SimCond)) {
    ...
    for (kk in 1:dim(nrdosk)[1]) {
      ...
      resk <- simTimeRecrudescenceCombo(model, parameters,
        ...
        log(LLOQ),
        simlength = max(dataGeneral$TIME) + 24 * 7,
        setting = setting
      )
  ...
  compounds <- within(dplyr::rename(compounds, Compound = NAME), {
    Compound <- gsub(" Dose", "", Compound)
  })
  ...
  maxValue <- ceiling(max(c(
    max(subset(TRdataMeanFinal, flagMin == FALSE)$Time2Recrud / 24),
  ...
  plot2D <- function(results, TRdataMeanFinal) {
    ...
    results.int <- plyr::ddply(
      results, ~ n1 + n2 + CONDITION + Compound1 + Compound2 + Scenario,
      function(x) {
        x.interp <- akima::interp(
          x = x$Dose1,
          y = x$Dose2,
          z = x$Time2Recrud,
    ...
    gr_x <- MMVbase::MMVggplot(results.int, aes(Dose1, Dose2),
      ...
      labs(
        x = compounds$Compound[compounds$ADM == 1],
  ...
  plotObsPred <- function(pred, obs) {
    ...
    gr <- gr + facet_wrap(~STRAT) +
      labs(
        ...
        caption = paste0(
          "Color indicates data from one individual (per panel)",
...
#'
#' @description Perform comparison of multiple models OBJ, AIC and BIC values
#' using likelihood ratio test, implemented in R as `pchisq()`
#' @param ModelPaths Character string of path to folder containing multiple IQRnlme projects
#' @param ModelName Character vector of desired names of models, in order.
#' If set to NULL, model names from IQRnlme projects will be used as-is. Default: `NULL`
#' @param outputFolder Default: '../04-Output/Sxxx-ModelComparison'
#' @param parameterPNG Character vector of parameter names for which to additionally save output as .png. Default: `NULL`

#' @return Nothing is returned directly by the function, visualizations of
#' model comparisons are saved as .pdf to `outputFolder`, and also saved as .png if parameters are defined in `parameterPNG`
...
likelihoodRatioTestIQR <- function(ModelPaths, ModelName = NULL,
    ...
    if (!failed) {
      ...
      for (j in 1:length(ProjectObject[[i]])) {
        ...
        if (!"try-error" %in% class(resModel_j)) {
          # OBJ, AIC, BIC
          goodnessOfFit <- c(
            "OBJ" = resModel_j$objectivefunction$OBJ,
          ...
          # Concatenate the parameters:
          parameters.All.Values <- c(
            parameters.FixedEffects.Values,
  ...
  for (i in 1:length(parameters.Extra)) {
    ...
    # Temporary Dataset:
    ResultData_temp <- data.frame(
      GROUP = ResultData$ModelID,
...
#'
#' @description Remove a single covariate model to a modelSpec object.
#' @param modelSpec modelSpec object in format generated by `modelSpec_IQRest`
#' @param covariateModelToAdd covariateModel and values to remove, see `modelSpec_IQRest`
#' @param COVcentering 	Named vector defining centering value for covariates i.e. COVcentering = c(WT0=55)
#' @return A list containing the IQR model spec, see `?modelSpec_IQRest`
#' @export
#' @author Mohammed H. Cherkaoui (MMV)
...
#'
#' @description Convert interaction parameters from an IQRnlmeProjectMulti containing
#' PD interaction models into interpretable fold-change values.
#' @param x An IQRnlmeProjectMulti object containing models to compare
#' @param filename Character string containing path to filename for writing results.
#' Default: `NULL`
#' @param FLAGout Logical: Return generated table? Default: `FALSE`
#' @param title Character string defining name of table. Default: `NULL`
#' @param footer Character string defining footer for table. Default: `NULL`
#' @return Results table if `FLAGout` = TRUE, otherwise table is saved to path provided
#' in `filename`
...
#' tablePopPDmalaria
#'
#' @description create a table with population estimates of PD parameters from an IQRnlmeProjectMulti object
#' @param x An IQRnlmeProjectMulti object containing models to compare
#' @return Table containing model parameters
#' @export
#' @author Aline Fuchs (MMV), Anne Kümmel (IntiQuan), Mohammed H. Cherkaoui (MMV)
#' @family Model Assessment
...
#' The VPC can subsequently be plotted using [plotVPC_IQRdataVPC].
#'
...
createDataVPC_IQRnlmeProject_MMV <- function(
  ...
  if (any(checkDupTimes__$DUP)) {
    ...
    data__ <- dplyr::mutate(data__,
      ...
      TIME = dplyr::case_when(
        YTYPE == 0 ~ TIME, # leave time for doses as is
  ...
  if (simtimeOption == "obs") {
    ...
    data__$gridtime <- {
      if (simtimeOption == "simTAD") data__$TAD else data__$TIME
    }
    ...
    allids <- unique(data__$ID)
    names(allids) <- unique(data__$ID) # id vector with id as names to get named output
  ...
  dataSim__ <- parallel::mclapply(1:Ntrials, mc.cores = ncores, mc.silent = FALSE, FUN = function(kkk__) {
    ...
    # Create event table
    eT__ <- IQReventTable(eDatak,
      regression = c(regNames__, parNamesk__),
      abs0inputs = ABS0inputs__, abs0Tk0param = namesTk0__,
      FLAGfixDoseOverlap = FLAGfixDoseOverlap
    )
    if (FLAGpreparePC) {
      eTpop__ <- IQReventTable(eDatakpop,
        regression = c(regNames__, parNamesk__),
        abs0inputs = ABS0inputs__, abs0Tk0param = namesTk0__,
        FLAGfixDoseOverlap = FLAGfixDoseOverlap
      )
    ...
    # .. Simulate -----
    simres <- sim_IQRmodel(
      model = model__, simtime = indSimTimes__, eventTable = eT__, FLAGoutputsOnly = TRUE,
    ...
    if (FLAGpreparePC) {
      simrespop <- sim_IQRmodel(
        model = model__, simtime = indSimTimes__, eventTable = eTpop__, FLAGoutputsOnly = TRUE,
        opt_initstep = opt_initstep,
        opt_abstol = opt_abstol,
        opt_reltol = opt_reltol,
        opt_maxstep = opt_maxstep,
    ...
    if (FLAGaddResidualNoise) {
      ...
      if (is.null(gpf)) {
        errParam__ <- lapply(
          getResults_IQRnlmeProject(project, FLAG_SAMPLE = FLAGsampleUncertainty)$additional$residualErrorModel,
          function(y) {
            z <- y$abcr
            z[is.na(z)] <- 0
            z
    ...
    simres <- stats::reshape(simres,
      direction = "long", varying = setdiff(names(simres), c("ID", "TIME")),
      timevar = "YTYPE"
    )
  ...
  if (FLAGpreparePC) {
    ...
    eTtyp__ <- IQReventTable(eDataTyp__,
      regression = c(regNames__, parNamesT__),
      abs0inputs = ABS0inputs__, abs0Tk0param = namesTk0__,
      FLAGfixDoseOverlap = FLAGfixDoseOverlap
    )
    ...
    simrestyp__ <- sim_IQRmodel(
      model = model__, simtime = indObsTimes__, eventTable = eTtyp__, FLAGoutputsOnly = TRUE,
      opt_initstep = opt_initstep,
      opt_abstol = opt_abstol,
      opt_reltol = opt_reltol,
      opt_maxstep = opt_maxstep,
...
#' can be a vector with paths to these additional IQRnlmeProjects. It is not allowed that
#' in these projects and modelSampleSimulate estimated parameter names overlap
#' @param modelSampleSimulate Default: NULL (Seems to be defunct)
...
#'
#' @param dataVPC Named list with simulated (sim) and potentially typical predictions for observed (obs) data
#' @param stratifyBy Column name used to stratify VPC plots. Default: `NULL`
#' @param filename Filename for export of the VPC to a PDF. If NULL object including plotting data is returned. Default: `NULL`
#' @param FLAGlogY logical. If TRUE the Y axis will be shown in logscale. Default: `FALSE`
#' @param FLAGuseTAD logical. If TRUE then observed time after previous dose (TAD) is used for x-axis. If FALSE then actual TIME is used. Default: `FALSE`
#' @param FLAGdataPlotOnly logical. If TRUE then no simulation is done and only the data and lines for the percentiles are plotted. This allows to find suitable settings for the binning parameters BIN.groupsize and BIN.lambda. Default: `FALSE`
#' @param FLAGnoDataPoints If TRUE then data points will be ommitted but percentiles for data will be shown. Default: `FALSE`
#' @param FLAGpc Flag whether to apply prediction correction. Can only be performed if typical prediction are present in dataVPC. Default: `FALSE`
#' @param FLAGplotBins Flag whether to plot the bin boundaries as vertical lines. Default: `FALSE`
#' @param FLAGplotN Flag whether to display number of subjects in panel. Default: `FALSE`
#' @param title Character string to be plotted as plot title (on each page). Default: `NULL`
#' @param subtitle Character string to be plotted as plot subtitle (on each page). Default: `NULL`
#' @param BIN.column Column name of integer column assigning the observations to time bins. Defaults to NULL such that bins are automatically generated or the user-provided BIN.breaks are used. Default: `NULL`
#' @param BIN.breaks Numerical vector with bin borders that are used for the observation times. Defaults to NULL such that bins are automatically generated. This input is ignored if BIN.column is provided. Default: `NULL`
#' @param BIN.groupsize Smallest expected group size for the binning. By default the round(0.5 x number of subjects) in the dataset is used. Default: `NULL`
#' @param BIN.lambda Penalization of intra-group variance, set to 1 to have more groups and set to 0 to get less but larger groups. Default: `NULL`
#' @param BIN.resolution Gaps between groups of data points greater than resolution lead to separation of groupsDefault: `NULL`
#' @param CIlevel Confidence interval level for percentiles (in percent). Default: `NULL`
#' @param percentiles numeric vector with 1, 2 or 3 entries between 0 and 100 to be used as percentiles for data and simulation plots. 1 entry: only this percentile is plotted, e.g. for 50 the median is visualized. 2 entries: low, and high percentiles were provided and median should also be plotted. 3 entries: low, medium and high percentiles were provided in this order. Default: `NULL`
#' @param alphaDataPoints Alpha transparency level for data points. Default: `NULL`
#' @param alphaPredInter Default: `NULL`
#' @param ActivityPath Default: `NULL`
#' @param Caption Default: `NULL`
#' @param setting Default: `NULL`
#' @return List of generated VPC figures
#' @export
#' @author Mohammed H. Cherkaoui (MMV)
#' @family Model Assessment
plotVPC_MMVdataVPC <- function(dataVPC, stratifyBy = NULL, filename = NULL, FLAGlogY = FALSE,
  ...
  if (n_Output > 1 && !is.null(xlimits) && length(xlimits) ==
    ...
    n_Output) {
    stop(
      "Number of elements in 'xlimits' (=", length(xlimits),
  ...
  if (n_Output > 1 && !is.null(ylimits) && length(ylimits) ==
    ...
    n_Output) {
    stop(
      "Number of elements in 'ylimits' (=", length(ylimits),
      ") is different that the number of output (=", n_Output,
      "): Please Adjust"
    )
  }
  if (n_Output > 1 && !is.null(xlabel) && length(xlabel) ==
    ...
    n_Output) {
    stop(
      "Number of elements in 'xlabel' (=", length(xlabel),
      ") is different that the number of output (=", n_Output,
      "): Please Adjust"
    )
  }
  if (n_Output > 1 && !is.null(ylabel) && length(ylabel) ==
    ...
    n_Output) {
    stop(
      "Number of elements in 'ylabel' (=", length(ylabel),
      ") is different that the number of output (=", n_Output,
      "): Please Adjust"
    )
  }
  grList <- plotVPC_IQRdataVPC(
    dataVPC = dataVPC, stratifyBy = stratifyBy,
  ...
  if (FLAGlabel) {
    for (i in seq_along(grList)) {
      ...
      names(grList[[i]]$content) <- labels[match(
        STRAT_value,
        levels
      )]
    }
  ...
  for (i in seq_along(grList)) {
    for (j in seq_along(grList[[i]]$content)) {
      ...
      if (!is.null(colorFill)) {
        ...
        labelNames <- c(
          paste0(min(percentiles), "th Percentile"),
    ...
    for (i in seq_along(grList)) {
      for (j in seq_along(grList[[i]]$content)) {
        ...
        if (is.null(stratifyBy)) {
          ...
          stratifyLabel_j <- gsub(paste0(
            stratifyBy,
            ": "
          ), "", unique(grList[[i]]$content[[j]]$data$STRAT))
          if (FLAGlabel) {
            stratifyLabel_j <- labels[match(
              stratifyLabel_j,
              levels
            )]
            grList[[i]]$content[[j]]$data$STRAT <- labels[match(
              grList[[i]]$content[[j]]$data$STRAT,
              levels
            )]
            for (k in seq_along(grList[[i]]$content[[j]]$layers)) {
              grList[[i]]$content[[j]]$layers[[k]]$data$STRAT <- labels[match(
                grList[[i]]$content[[j]]$layers[[k]]$data$STRAT,
                levels
              )]
            }
          }
          if (opt.layout$nrow == 1 && opt.layout$ncol ==
            ...
            grList[[i]]$content[[j]] <- grList[[i]]$content[[j]] +
              theme(
                ...
                stratifyName,
                ": ", stratifyLabel_j
              ), subtitle = grList[[i]]$title)
          } else {
            if (is.null(title)) {
              title <- gsub(
                "Visual Predictive Check",
                paste0(
                  "Visual Predictive Check by ",
                  stratifyName
                ), grList[[i]]$title
              )
            }
    ...
    for (i in seq_along(grList)) {
      if (toupper(style) == "IQR") {
        if (length(grList) == 1) {
          ...
          IQRoutputFigure(x = grList[[i]], filename = paste0(gsub(
            ".pdf",
            "", filename
          ), "_", i, ".pdf"))
        }
        ...
        if (length(grList) == 1) {
          ...
          IQRoutputFigure(
            x = grList[[i]], filename = paste0(gsub(
              ".pdf",
              "", filename
            ), "_", i, ".pdf"), title = title,
...
plotSummaryComparisonBinary <- function(plotData,
  ...
  gglist <- lapply(seq_along(variableNames), function(k) {
    ...
    gg <- MMVbase::MMVggplot(thisD) +
      ...
      ) + # Add error bars with dodge position
      ...
      geom_text(
        aes(
          label = paste0(k, "/", N), x = get(compCategory),
          y = structure(c(105)),
          col = get(sourceCol)
        ),
        position = position_dodge(width = -0.5),
        show.legend = FALSE, size = 3
      ) + # Add text labels for N
      labs(
        x = NULL,
        y = NULL,
        title = thisVariable
      ) + # Set title to the current variable
