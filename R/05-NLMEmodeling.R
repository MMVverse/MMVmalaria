...
testPKmodelsIQR_testing <- function(testSetup, data, parOptions = list(),
  ...
  Lag          <- ifelse(testSetup$LagTime                               , 1, 0)
  ...
  for (kmod in 1:length(modelsTest)) {
    ...
    # Adjust dosing and model spec based on testsetup$LagTime 
    for (klt in 1:length(testSetup$LagTime)) {
      dosing_k <- dosing
      if (!testSetup$LagTime[klt]) {
        # lag OFF: remove Tlag1 from ALL modelSpec vectors and from dosing
        rm_nm <- "Tlag1"
        POPvalues0k      <- POPvalues0k[ setdiff(names(POPvalues0k),      rm_nm) ]
        POPestimatek     <- POPestimatek[ setdiff(names(POPestimatek),     rm_nm) ]
        IIVvalues0k      <- IIVvalues0k[ setdiff(names(IIVvalues0k),      rm_nm) ]
        IIVestimatek     <- IIVestimatek[ setdiff(names(IIVestimatek),     rm_nm) ]
        IIVdistributionk <- IIVdistributionk[ setdiff(names(IIVdistributionk), rm_nm) ]
        
        # If Tlag is present in dosing object, remove it  
        if ("Tlag" %in% names(dosing_k$INPUT1)) {
          dosing_k$INPUT1 <- dosing_k$INPUT1[ setdiff(names(dosing_k$INPUT1), "Tlag") ]
        }
        ...
        for (kcovn in 1:length(covarianceModel)) {
          ...
          for (kcovtn in 1:length(covariateModel)) {
            ...
            ModelInfo <- within(ModelInfo, {
              ...
              LagTime[modelIdx] <- ifelse(testSetup$LagTime[klt], "with lag time", "no lag time")
