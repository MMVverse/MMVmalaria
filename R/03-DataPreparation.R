...
summarizeReplicates <- function(data, name) {

  # Get Attributes:
  if ("IQRdataGENERAL" %in%  class(data))
    attr0 <- attributes0(data)

  # Check whether valid obsName given with name:
  if (!(name %in% unique(data$NAME)))
    stop(paste0("'", name, "' not a observation in data set."))

  # Divide into names to summarize and names not to summarize:
  data0 <- subset(data, !(NAME %in% name))
  data1 <- subset(data, NAME %in% name)

  # Summarize:
  data1s <- plyr::ddply(data1, ~USUBJID+NAME+TIME, function(x) {
    y <- x[1,]
    y$VALUE <- mean(x$VALUE, na.rm = TRUE)
    y$DV <- mean(x$DV, na.rm = TRUE)
    y
  })
  if ("IQRdataGENERAL" %in%  class(data))
    data1s$IXGDF <- max(data$IXGDF) + 1:length(data1s$IXGDF)

  # Concatenate:
  out <- rbind(data0, data1s)

  # Sort:
  out <- out[order(out$USUBJID, out$TIME, out$NAME), ]
